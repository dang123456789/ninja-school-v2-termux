...
public class Lucky extends Thread {
    ...
    protected synchronized void joinLuck(Player p, int joinxu) throws IOException {
        ...
        if (joinxu >= 1000000 && p.c.isTaskDanhVong == 1 && p.c.taskDanhVong[0] == 0 && p.c.taskDanhVong[1] < p.c.taskDanhVong[2]) {
            p.c.taskDanhVong[1]++;
            if (p.c.taskDanhVong[1] == p.c.taskDanhVong[2]) {
                p.sendAddchatYellow("Bạn đã hoàn thành nhiệm vụ danh vọng.");
            }
        }
        if (joinxu >= 10000 && p.c.isTaskDanhVong == 1 && p.c.taskDanhVong[0] == 1 && p.c.taskDanhVong[1] < p.c.taskDanhVong[2]) {
            p.c.taskDanhVong[1]++;
            if (p.c.taskDanhVong[1] == p.c.taskDanhVong[2]) {
                p.sendAddchatYellow("Bạn đã hoàn thành nhiệm vụ danh vọng.");
            }
        ...
        LichSu.LichSuXu(p.c.name, p.c.xu, p.c.xu - joinxu, " Đặt Vòng Xoay May Mắn", -joinxu);
        this.totalxu += joinxu;
    ...
    private void turned() throws Exception {
        ...
        if (this.numplayers > 1) {
            xuwin = xuwin * 95L / 100L; // 10%
        }
        this.numplayers = 0;
        this.totalxu = 0;
        Char ns = Client.gI().getNinja(p.name);
        if (ns != null) {
            ns.upxuMessage(xuwin * 1L);
             LichSu.LichSuXu(ns.name, ns.xu, ns.xu + xuwin, "Thắng Vòng Xoay May Mắn", + xuwin);
        } else {
