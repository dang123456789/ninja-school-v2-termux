 ...
 */
public class UpgradeTonNgoKhong {

    public static void UpgradeTonNgoKhong(Player p, Item item, int type) throws IOException {
        ...
        UpgradeTonNgoKhong.UpgradeTonNgoKhongOptions(p, item, type);
    ...
    public static void UpgradeTonNgoKhongOptions(Player p, Item item, int type) {
        try {
            int upPer = GameSrc.tileup[item.upgrade];
            if (Util.nextInt(100) < upPer) {
                p.c.removeItemBody((byte) 11);
                Item itemup = ItemTemplate.itemDefault(834, true);
                itemup.quantity = 1;
